...
def CategoryView(request, cats):
    # templates |slugify將空格轉成'-', 傳入views 變成 coding-tutorial,
    # templates |slugify將空格轉成'%', 傳入views 變成 coding%tutorial, 不需調整replace
...
def LikeView(request, pk):
    ...
    if post.likes.filter(id=request.user.id).exists():
        ...
        post.likes.add(request.user) # Post model # request 取得前台資料
        liked = True
